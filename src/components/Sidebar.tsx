...
export function Sidebar({ collapsed, mobileOpen, onToggle, onMobileClose }: SidebarProps) {
  ...
  return (
    <>
      ...
      <aside
        className={cn(
          "fixed left-0 top-0 h-screen z-50 flex flex-col transition-transform duration-300 ease-out",
          "border-r border-[var(--glass-border)]",
          "bg-[var(--bg-primary)] w-[280px]",
          "md:hidden",
      ...
      >
        ...
        <div className="flex items-center justify-between h-16 px-4 border-b border-[var(--glass-border)]">
          ...
          <button
            onClick={onMobileClose}
            className="p-2 rounded-[var(--radius-md)] text-[var(--text-tertiary)] hover:text-[var(--text-primary)] hover:bg-[rgba(255,255,255,0.04)]"
          >
            <X size={18} />
          </button>
        </div>
